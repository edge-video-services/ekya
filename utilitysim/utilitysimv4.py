            ...
            if i == len(res_change_times) - 1:
                ...
                if allocation != 0:
                    if total_remaining_res_time/allocation < block_duration:
                        ...
                        assert total_remaining_res_time >= -0.001, "Remaining res time: {}".format(total_remaining_res_time)
...
class MultiPeriodSimulator(object):
    ...
    def get_job_cfgs(self, task_id):
        ...
        for camera in self.cameras:
            ...
            camera.generate_oracle_configurations(task_id)   # Generate new training configs
            train_configs = camera.get_training_configurations()
            oracle_configs = camera.get_oracle_configurations()
            job_pairs.append((camera.get_inference_job(), train_configs, oracle_configs))
        return job_pairs
    ...
    def step_till_completion(self):
        ...
        for task_id in self.task_ids:
            ...
            sched_job_pairs = [(jc[0], jc[1]) for jc in job_cfgs]   # Training only
            oracle_job_pairs = [(jc[0], jc[2]) for jc in job_cfgs]    # Oracle
            period_allocation = self.scheduler(sched_job_pairs, self.total_resources, self.retraining_period)
...
def generate_config(name, final_accuracy, job_time, start_accuracy, inference_job=None, oracle=False):
    if start_accuracy > final_accuracy:
        if oracle:
            print("WARNING: {} model is already trained. start_accuracy {} > final_accuracy {}. Still adding..".format(name, start_accuracy,                                                                                   final_accuracy))
        else:
            raise Exception("WARNING: {} model is already trained. start_accuracy {} > final_accuracy {}. This is invalid.".format(name, start_accuracy,
                                                                                                     final_accuracy))
    init_time = 0
...
class Camera(object):
    def __init__(self, name, taskwise_training_profiles, taskwise_oracle_profiles, taskwise_start_accuracy, inference_job=None,
                 inference_max_resources = 1, inference_profile_path = 'real_inference_profiles.csv',
                 inference_camera_profile='c1', start_task_id = "1"):
        '''
        ...
        if inference_job:
            ...
            self.inference_job = InferenceJob("{}_inference".format(self.name),
                                              ...
                                              get_infer_profile(max_inference_resources=inference_max_resources,
                                                                camera=inference_camera_profile,
    ...
    def generate_training_configurations(self, task_id):
        ...
        all_hp_ids = [p[2] for p in self.taskwise_training_profiles[task_id]] # Format is final_acc, resource_time, hp_id
        assert len(set(all_hp_ids)) == len(all_hp_ids), "hp_ids are not unique."
        for final_acc, resource_time, hp_id in self.taskwise_training_profiles[task_id]:
            name = "{}_train_{}".format(self.name, hp_id)   # Format of hp_id is hpid_epochs
            if final_acc < self.current_accuracy:
                print("WARNING(predictedcfg): Final accuracy {} less than current acc {} for camera {}, ignoring profile {}.".format(final_acc, self.current_accuracy, self.name, name))
            else:
                configs.append(generate_config(name,
                             final_acc,
                             resource_time,
                             self.current_accuracy,
                             self.inference_job)) # no inference job because this is used just for scheduling
    ...
    def generate_oracle_configurations(self, task_id):
        ...
        # Generates and returns the valid training configurations (final acc > current_acc)
        # TODO(romilb): Currently not bothering with profile curve, just start and end acc.
        all_hp_ids = [p[2] for p in self.taskwise_oracle_profiles[task_id]]  # Format is final_acc, resource_time, hp_id
        assert len(set(all_hp_ids)) == len(all_hp_ids), "hp_ids are not unique."
        for final_acc, resource_time, hp_id in self.taskwise_oracle_profiles[task_id]:
            if final_acc < self.current_accuracy:
                print("WARNING(oracle): Final accuracy {} less than current acc {} for camera {}, still adding profile if the predictor decides to use it..".format(final_acc, self.current_accuracy, self.name))
