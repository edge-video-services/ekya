...
def thief_scheduler(training_jobs, inference_jobs, total_resources, **kwargs):
    ...
    for candidate_job in all_jobs:
        for victim_job in [x for x in all_jobs if x != candidate_job]:
            ...
            for steal_amount in np.arange(0, max_steal_resources+0.00001, 0.1):
                if should_steal:
...
def single_camera_naive(training_configs, retraining_period, training_res_alloc, allow_no_train=True):
    '''
    A dumb variant of Single Camera Optimizer. Picks a training configuration which fits in the retraining window
...
def thief_sco_scheduler(job_pairs, total_resources, retraining_period, iterations = 10):
    '''
...
class MultiPeriodSimulator(object):
    ...
    def __init__(self, cameras, total_resources, scheduler, retraining_period,
                 num_periods = 1, incomplete_jobs_checkpoint = False):
    ...
    def get_job_pairs(self):
        ...
        for camera in self.cameras:
            camera.generate_training_configurations()   # Generate new training configs
    ...
    def step_till_completion(self):
        ...
        for p in range(self.num_periods):
            print("Running period {}".format(p))
            # TODO: Get training profiles here
            job_pairs, train_to_camera_map = self.get_job_pairs()
            # TODO: Get new period allocation here given inference job and training profiles from Camera object
            period_allocation = self.scheduler(job_pairs, self.total_resources, self.retraining_period)
            period_allocation_log[p] = period_allocation
            ...
            sim = simulator(training_jobs, inference_jobs, self.total_resources,
                            retraining_period=self.retraining_period,
                            sim_name='period{}'.format(p),
                            period_allocation=period_allocation)
            results[p] = sim.step_till_completion()

            # TODO: Get metrics from all inference jobs and update the configurations in cameras and global result
            # TODO: Use training jobs to update accuracy or use inference jobs to update? Key question - do incomplete training jobs checkpoint at the end of retraining period?
        ...
        return results, period_allocation_lognsp
...
class Camera(object):
    def __init__(self, name, training_profiles, start_accuracy, inference_job=None):
        '''
        inference_job: InferenceJob object
        training_profiles: list of 3 tuples - [(final_acc, resource_time, start_acc), ..]
        '''
        self.name = name
        self.training_profiles = training_profiles
        self.current_accuracy = start_accuracy
        self.configs = []
        if inference_job:
            self.inference_job = inference_job
        else:
    ...
    def generate_training_configurations(self):
        configs = []
        # Generates and returns the valid training configurations (final acc > current_acc)
        for i, (final_acc, resource_time, _) in enumerate(self.training_profiles):
            if final_acc < self.current_accuracy:
                ...
                configs.append(generate_config2("{}_train_{}".format(self.name, i),
                             final_acc,
                             resource_time,
                             self.current_accuracy,
                             self.inference_job))
