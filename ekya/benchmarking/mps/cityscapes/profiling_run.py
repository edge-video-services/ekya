...
def profiling_run(args):
    ...
    hyperparameter_id = args["hyperparameter_id"]
    override_batch_size = args["override_batch_size"]

    hyp_map = {str(idx): hyp for idx, hyp in enumerate(HYPERPARAM_LIST)}

    hyperparam = hyp_map[hyperparameter_id]
    if override_batch_size:
        hyperparam["batch_size"] = override_batch_size
    ...
    if pretrained_sample_names:
        ...
        print("Pretrained dataset size: {}".format(len(dataset_for_pretrained)))
    ...
    for task in range(num_tasks):
        ...
        print("Task {}/{}, hyperparam {}".format(task, num_tasks, hyperparameter_id))
        ...
        if task == 0:
            ...
            print("Pretrained samples: {}, subsample:".format(len(pretrained_subsample_idxs)))
            num_samples_to_pick = max(int(len(pretrained_subsample_idxs) * subsample), 2)    # Pick atleast two samples.
            print("Picking samples: {}".format(num_samples_to_pick))
            ...
            print("Subsampling done. Task {}. Total train data: {}. Total val data: {}. Pretrain_data: {}".format(task, len(task_train_dataset), len(task_val_dataset), len(pretrained_subsampled_dataset)))
        ...
        e2e_times.append([GPU_ALLOC, task, e2e_time])
        print("Task {} done.\nVal acc:\t{}".format(task, best_val_acc))
        results[hyperparameter_id]["val_acc"][task] = best_val_acc
        profiling_results[hyperparameter_id][task] = profile
        subprofiling_test_results[hyperparameter_id][task] = subprofile_test_results
    ...
    return results, profiling_results, e2e_times
